...
- 默认在所选工作屏幕内搜索目标图片；也可以手动框选一小块搜索范围，减少误判和 CPU 占用
...
- 先在 `工作屏幕` 下拉框里选择要监控的屏幕；后续截图、匹配和点击都只在这个屏幕内执行
- 点击 `截取目标样式` 后，主窗口不会隐藏；程序会截取所选屏幕并打开预览窗口，在预览图上拖框，松开后自动保存模板图片
- 默认勾选 `在所选屏幕内全屏搜索目标图片`，窗口在这个屏幕内漂移也能按图片样式找到目标
- 默认点击 `匹配图片中心`，不依赖固定屏幕坐标
- `启用自动点击` 是独立开关：关闭时只识别和记日志，打开后才真的移动鼠标点击
- 如果误判，可以点击 `拖动限制搜索范围`，同样在所选屏幕的预览图上框选，只在某一块区域里搜索图片样式
